...
import com.zennetvpn.ShanDevzService.logger.*;
...
public class SSLProxy implements ProxyData
{
	...
	class HandshakeTunnelCompletedListener implements HandshakeCompletedListener {
        ...
        public void handshakeCompleted(HandshakeCompletedEvent handshakeCompletedEvent) {
			...
			//	try {
			//		SkStatus.logInfo("SSL Info: " + session.getPeerPrincipal().getName().substring(session.getPeerPrincipal().getName().indexOf(",") + 1));
			//	} catch (SSLPeerUnverifiedException e) {
			//		SkStatus.logInfo("SSL Info: None");
			//	}
	...
	public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException
	{
		...
		if (mSocket.isConnected())
		{
			...
			/*	if (httpReponseFirstLine.startsWith("HTTP/1.1 101")){
			 SkStatus.logInfo("<b>HTTP/1.1 101 CloudshareNet VPN Fast&Private</b>");
			 } else if (httpReponseFirstLine.startsWith("HTTP/1.1 200")){
			 SkStatus.logInfo("<b>HTTP/1.1 200 CloudshareNet VPN Fast&Private</b>");
			 } else {
			 SkStatus.logInfo("<b>"+httpReponseFirstLine+"</b>");
			 }
			...
			if (parseInt == 200) {
				...
				SkStatus.logInfo("<b>HTTP/1.1 101 Switching Protocols</b>");
				return this.mSocket;
			}
