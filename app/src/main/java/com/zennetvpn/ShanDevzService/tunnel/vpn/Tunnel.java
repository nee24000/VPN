...
package com.zennetvpn.ShanDevzService.tunnel.vpn;
...
public class Tunnel
{
	...
	private boolean startVpn(boolean forwardDns, String[] dnsResolver, String[] excludeIps,
							 boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering, boolean useBypass) throws Exception
	{
		...
		try
		{
			...
			if (Build.VERSION.SDK_INT >= 21) {
				if (useBypass) {
					mHostService.onDiagnosticMessage("Activating SlowDNS, cause your server and payload has SlowDNS setup.");
			...
			tunFd = builder
				...
				.setConfigureIntent(ShanDevzService.getGraphPendingIntent(mHostService.getContext()))
				.establish();
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
					@Override
					public void onStart(){
						mHostService.onDiagnosticMessage("DNS Forwarding Enabled");
					}
					@Override
					public void onStop(){
						//mHostService.onDiagnosticMessage("Spping D forwarding");
						stop();
					}
				});
