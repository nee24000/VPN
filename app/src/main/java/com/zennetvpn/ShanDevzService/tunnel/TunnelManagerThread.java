...
import com.zennetvpn.ShanDevzService.tunnel.vpn.TunnelVpnService;
import androidx.localbroadcastmanager.content.LocalBroadcastManager;
import java.util.List;
import com.zennetvpn.ShanDevzService.tunnel.vpn.VpnUtils;
import android.util.Log;
import com.zennetvpn.ShanDevzService.tunnel.vpn.TunnelState;
import android.content.Intent;
import com.zennetvpn.ShanDevzService.tunnel.vpn.TunnelVpnSettings;
import android.content.BroadcastReceiver;

import com.zennetvpn.ShanDevzService.tunnel.vpn.TunnelVpnManager;
...
import com.zennetvpn.ShanDevzService.config.PasswordCache;
			...
			public void run() {
				while (true) {
					...
					if (lastPingLatency > 0) {
						SkStatus.logInfo(String.format("%d ms", lastPingLatency));
						break;
					}
				}
			}
	...
	*/
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
			...
			.replace(mConfig.getPrivString(Settings.SERVIDOR_KEY), "**********")
			.replace(mConfig.getPrivString(Settings.PROXY_IP_KEY), "**********"));*/
			mConnected = false;
			throw e;
		}
	}
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
            ...
            if (mConfig.ssh_compression()){
                ...
                SkStatus.logInfo("<strong>Data Compression Enabled</strong>");
			}
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                ...
                if (proxy != null) {
                    SkStatus.logInfo("<strong>Access point name (APN) net proxy:</strong> " + String.format("%s:%d", proxy.getHost(), proxy.getPort()));
                }
            }
            ...
            SkStatus.logError(e.getCause().toString());
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				if (mConnection.authenticateWithPassword(usuario,
						senha)) {
					SkStatus.logInfo("<b>Authenticated</b>");
				}
			}
			...
			Log.e(TAG, "Problem during handle Authentication()", e);
		}
			...
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				if (f.exists()) {
					...
					if (mConnection.authenticateWithPublicKey(usuario, f,
							senha)) {
						SkStatus.logInfo("Forward Successful");
					}
				}
			}
		} catch (Exception e) {
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("User or Password Expired");
	...
	throws Exception {
		...
		SkStatus.logInfo("<strong>Finger Print: </strong>"  +  fingerPrint);
			...
			switch (mTunnelType) {
				...
				case Settings.bTUNNEL_TYPE_SSL_TLS:
                    ...
                    try{
                     ...
                     //       SkStatus.logInfo("Sproof Server: " + customSNI);
...
// wala nang silbi
                ...
                case Settings.bTUNNEL_TYPE_PAY_SSL:
                    ...
                    String sshServer2 = mConfig.getPrivString(Settings.PROXY_IP_KEY);
                    int sshPort2 = Integer.parseInt(mConfig.getPrivString(Settings.PROXY_PORTA_KEY));

                    try{
                        
                   //     if (customSNI2 != null && !customSNI2.isEmpty() && ! isProteger)
                    //        SkStatus.logInfo("Sproof Server : " + customSNI2);
	...
	protected void startTunnelVpnService() throws IOException {
		...
		SkStatus.logInfo("Tunneling service started");
		SharedPreferences prefs = mConfig.getPrefsPrivate();
		...
		String servidorIP = mConfig.getPrivString(Settings.PROXY_IP_KEY);
         /*   try {
				String servidorIP = mConfig.getPrivString(Settings.PROXY_IP_KEY);
			} catch(Exception e) {
				SkStatus.logError(R.string.error_proxy_invalid);
